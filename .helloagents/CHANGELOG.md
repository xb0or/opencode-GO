...
- `UPSTREAM_TIMEOUT` ????? `0`????????????????????????????????
- ????? `context canceled` ?????????499??? `context deadline exceeded` ????????504????? 502/context ????????? KEY ???

## 2026-06-22

- ??????????API ????????GO ?????? group ??? `go`?????????
- Go ????????????????????? 1 ????? 7 ???? 30 ????????? Token????? `checkedAt + resetInSec` ???????
- KEY ???? 429 ??? quota-aware ??????????? rolling/weekly/monthly bucket ????/??? 100% ?????????????? rolling ??????????????????
- ??????????????? 1 ?????? Token ?????????? pool ????? `go test ./...` ???

## 2026-06-21

- API 密钥页 Go 限额展示补齐滚动/每周/每月三类使用量与重置时间，并持久化最近一次查询结果到数据库，刷新页面后仍可展示。
- 使用记录日志详情新增错误详情区块；同协议上游 4xx/5xx 响应会记录解析后的具体错误信息，便于排查失败原因。
- 修复管理后台“模型路由”页面主内容缺失导致切换后空白的问题，恢复模型列表、同步、添加、设置与删除入口。
- 将 API 密钥表格的 Go 限额“查询/刷新”按钮移动到操作栏，Go 限额列仅展示当前限额状态、错误提示或未配置状态。
