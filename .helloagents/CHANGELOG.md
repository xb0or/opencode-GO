...
- 修复管理后台静态资源被 Cloudflare/浏览器缓存后出现 HTML 与 ES Module 版本不一致的问题，避免新版使用记录模板调用旧版 `app.js` 时触发 `usageTokenLabel is not a function`。
- `/admin` 嵌入式静态资源统一返回 `Cache-Control: no-store` 等防缓存头，并为 `admin.css`、`app.js` 与内部模块 import 增加版本查询参数。
